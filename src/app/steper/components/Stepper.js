...
const Stepper = ({ steps, currentStep }) => {
    ...
    const displaySteps = newStep.map((step, index) => {
        console.log("stepcom", step.selected)
        return (
            <div key={index} className={index !== newStep.length - 1 ? 'flex items-center w-full' : 'flex items-center'}>
                <div className='relative flex flex-col items-center text-teal-600'>
                    <div className={`flex items-center justify-center w-12 h-12 py-3 transition duration-500 ease-in-out border-2 border-gray-300 rounded-full ${step.selected ? "bg-green-600 text-white font-bold border border-green-600" : ""}`}>
                        {/* Display number  */}
                        {
                            step.completed ? (
